 ...
 * 外部 App 数据查询 Service。
 *
 * 通过 startService + PendingIntent 回传结果的方式，允许 RikkaHub 等 App 跨进程读取
 * DidaDida 的打卡数据。和 Termux 的 RUN_COMMAND 模式完全一致。
 *
 * 支持的 Action:
 ...
 */
class DataQueryService : android.app.Service() {

    /** 全局协程作用域，用于在 Service 中执行挂起函数 */
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        serviceScope.launch {
                ...
                "QUERY_TODAY" -> {
                    ...
                    Bundle().apply {
                        ...
                        putString("data", json.encodeToString(
                            mapOf("log" to log, "summary" to summary)
                        ))
                    }
                }
                else -> Bundle().apply {
            ...
            // 通过 PendingIntent 回传结果，和 Termux 的 RUN_COMMAND 一模一样
            if (pi != null) {
                try {
                    pi.send(applicationContext, 0, Intent().apply {
